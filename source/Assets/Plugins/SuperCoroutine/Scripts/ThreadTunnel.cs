    ...
    public class ThreadTunnel : MonoBehaviour
    {
        ...
        private static ThreadTunnel Instance
        {
            get
            {
                if (_instance == null)
                {
                    var instanceHome = GameObject.Find("TimeTunnel Controller");
        ...
        private void OnDestroy()
        {
            for (var i = 4; i >= 0; i--)
            {
                for (var j = 0; j < _threadSpindles[i].Count; j++)
                {
            ...
            for (var i = 0; i < _looseThreads.Count; i++)
            {
        ...
        private static void AddItemToProcessQueue(ThreadData chosenThread, Process process)
        {
            ...
            {
                var newProcesses = new Process[chosenThread.Processes.Length + ProcessesBlockSize];
                var j = 0;
                int i;
        ...
        private static void SelectThreadFromSpindle(System.Threading.ThreadPriority priority)
        {
            ...
            var spindle = Instance._threadSpindles[(int)priority];

            if (spindle.Count < SystemInfo.processorCount)
            {
                for (var i = 0; i < spindle.Count; i++)
                {
            ...
            var bestThreadCount = int.MaxValue;
            _chosenThread = null;
            ...
            for (var i = 0; i < spindle.Count; i++)
            {
                ...
                var count = spindle[i].Head - spindle[i].Tail;
                if (count < 0)
                {
        ...
        {
            for (var i = 0; i < Instance._looseThreads.Count; i++)
            {
        ...
        private static void ThreadProcess(object input)
        {
            ...
            var threadData = (ThreadData)input;
