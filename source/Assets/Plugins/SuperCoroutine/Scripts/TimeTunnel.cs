//============================================================
// Project: SuperCoroutines
// Author: Zoranner@ZORANNER
// Datetime: 2021-08-18 17:43:33
// Description: TODO >> Base on More Effective Coroutines Pro v3.12.0
//============================================================
...
namespace KimoTech.SuperCoroutines
{
    public class TimeTunnel : MonoBehaviour
    {
        ...
        private static readonly bool _tmpBool;
        private static Segment _tmpSegment;
        private static readonly CoroutineHandle _tmpHandle;
        ...
        private static readonly TimeTunnel[] ActiveInstances = new TimeTunnel[16];
        private static TimeTunnel _instance;
        public static TimeTunnel Instance
        {
            get
            {
                if (_instance == null || !_instance.gameObject)
                {
                    var instanceHome = GameObject.Find("Timing Controller");
                    ...
                    _instance = instanceHome.GetComponent<TimeTunnel>() ?? instanceHome.AddComponent<TimeTunnel>();
        ...
        private void Update()
        {
            ...
            {
                var coindex = new ProcessIndex { seg = Segment.SlowUpdate };
            ...
            {
                var coindex = new ProcessIndex { seg = Segment.RealtimeUpdate };
                if (UpdateTimeValues(coindex.seg))
                {
            ...
            {
                var coindex = new ProcessIndex { seg = Segment.Update };
                if (UpdateTimeValues(coindex.seg))
                {
        ...
        private void FixedUpdate()
        {
            ...
            {
                var coindex = new ProcessIndex { seg = Segment.FixedUpdate };
                if (UpdateTimeValues(coindex.seg))
                {
        ...
        private void LateUpdate()
        {
            ...
            {
                var coindex = new ProcessIndex { seg = Segment.LateUpdate };
                if (UpdateTimeValues(coindex.seg))
                {
        ...
        public void TriggerManualTimeframeUpdate()
        {
            ...
            {
                var coindex = new ProcessIndex { seg = Segment.ManualTimeframe };
                if (UpdateTimeValues(coindex.seg))
                {
        ...
        private void OnEditorUpdate()
        {
            ...
            if (EditorApplication.isPlayingOrWillChangePlaymode)
            {
                for (var i = 0; i < _nextEditorUpdateProcessSlot; i++)
                {
                    EditorUpdateProcesses[i] = null;
                }

                _nextEditorUpdateProcessSlot = 0;
                for (var i = 0; i < _nextEditorSlowUpdateProcessSlot; i++)
                {
            ...
            {
                var coindex = new ProcessIndex { seg = Segment.EditorSlowUpdate };
                if (UpdateTimeValues(coindex.seg))
                {
            ...
            {
                var coindex = new ProcessIndex { seg = Segment.EditorUpdate };
                if (UpdateTimeValues(coindex.seg))
                {
        ...
        private System.Collections.IEnumerator _EOFPump()
        {
            while (_nextEndOfFrameProcessSlot > 0)
            {
                ...
                var coindex = new ProcessIndex { seg = Segment.EndOfFrame };
                _EOFPumpRan = true;
                if (UpdateTimeValues(coindex.seg))
                {
            ...
            else if (IsRunning(handle))
            {
                switch (behaviorOnCollision)
                {
                    ...
                    case SingletonBehavior.Wait:
                        var newCoroutineHandle = Instance.RunCoroutineInternal(coroutine, Segment.Update, 0, false, null,
            ...
            else if (Instance._layeredProcesses.ContainsKey(layer))
            {
                ...
                switch (behaviorOnCollision)
                {
                    ...
                    case SingletonBehavior.AbortAndUnpause:
                        ...
                        break;
                    case SingletonBehavior.Wait:
                        var newCoroutineHandle = Instance.RunCoroutineInternal(coroutine, Segment.Update, layer, true, null,
                            new CoroutineHandle(Instance._instanceID), false);
            ...
            else if (Instance._taggedProcesses.ContainsKey(tag))
            {
                ...
                switch (behaviorOnCollision)
                {
                    ...
                    case SingletonBehavior.AbortAndUnpause:
                        ...
                        break;
                    case SingletonBehavior.Wait:
                        var newCoroutineHandle = Instance.RunCoroutineInternal(coroutine, Segment.Update, 0, false, tag,
                            new CoroutineHandle(Instance._instanceID), false);
        ...
        public static CoroutineHandle RunCoroutineSingleton(IEnumerator<float> coroutine, int layer, string tag, SingletonBehavior behaviorOnCollision)
        {
            ...
            {
                var matches = new List<CoroutineHandle>();
                var matchesEnum = Instance._taggedProcesses[tag].GetEnumerator();
                while (matchesEnum.MoveNext())
                {
                ...
                if (matches.Count > 0)
                {
                    var newCoroutineHandle = _instance.RunCoroutineInternal(coroutine, Segment.Update, layer, true, tag,
            ...
            else if (IsRunning(handle))
            {
                switch (behaviorOnCollision)
                {
                    ...
                    case SingletonBehavior.AbortAndUnpause:
                        ...
                        return handle;
                    case SingletonBehavior.Wait:
                        var newCoroutineHandle = Instance.RunCoroutineInternal(coroutine, segment, 0, false, null,
                            new CoroutineHandle(Instance._instanceID), false);
                        WaitForOtherHandles(newCoroutineHandle, handle, false);
                        return newCoroutineHandle;
            ...
            else if (Instance._layeredProcesses.ContainsKey(layer))
            {
                ...
                switch (behaviorOnCollision)
                {
                    ...
                    case SingletonBehavior.AbortAndUnpause:
                        ...
                        break;
                    case SingletonBehavior.Wait:
                        var newCoroutineHandle = Instance.RunCoroutineInternal(coroutine, segment, layer, true, null,
                            new CoroutineHandle(Instance._instanceID), false);
                        WaitForOtherHandles(newCoroutineHandle, _instance._layeredProcesses[layer], false);
                        return newCoroutineHandle;
                }
            ...
            else if (Instance._taggedProcesses.ContainsKey(tag))
            {
                ...
                switch (behaviorOnCollision)
                {
                    ...
                    case SingletonBehavior.AbortAndUnpause:
                        ...
                        break;
                    case SingletonBehavior.Wait:
                        var newCoroutineHandle = Instance.RunCoroutineInternal(coroutine, segment, 0, false, tag,
                            new CoroutineHandle(Instance._instanceID), false);
                        WaitForOtherHandles(newCoroutineHandle, _instance._taggedProcesses[tag], false);
                        return newCoroutineHandle;
                }
            ...
            SingletonBehavior behaviorOnCollision)
        {
            ...
            else if (behaviorOnCollision == SingletonBehavior.Wait)
            {
                ...
                if (matches.Count > 0)
                {
                    var newCoroutineHandle = _instance.RunCoroutineInternal(coroutine, segment, layer, true, tag,
                        new CoroutineHandle(_instance._instanceID), false);
                    WaitForOtherHandles(newCoroutineHandle, matches, false);
                    return newCoroutineHandle;
                }
            ...
            else if (_handleToIndex.ContainsKey(handle) && !CoindexIsNull(_handleToIndex[handle]))
            {
                switch (behaviorOnCollision)
                {
                    ...
                    case SingletonBehavior.AbortAndUnpause:
                        ...
                        return handle;
                    case SingletonBehavior.Wait:
                        var newCoroutineHandle = RunCoroutineInternal(coroutine, Segment.Update, 0, false, null,
            ...
            else if (_layeredProcesses.ContainsKey(layer))
            {
                ...
                switch (behaviorOnCollision)
                {
                    ...
                    case SingletonBehavior.AbortAndUnpause:
                        ...
                        break;
                    case SingletonBehavior.Wait:
                        var newCoroutineHandle = RunCoroutineInternal(coroutine, Segment.Update, layer, true, null,
                            new CoroutineHandle(_instanceID), false);
            ...
            else if (_taggedProcesses.ContainsKey(tag))
            {
                ...
                switch (behaviorOnCollision)
                {
                    ...
                    case SingletonBehavior.AbortAndUnpause:
                        ...
                        break;
                    case SingletonBehavior.Wait:
                        var newCoroutineHandle = RunCoroutineInternal(coroutine, Segment.Update, 0, false, tag,
                            new CoroutineHandle(_instanceID), false);
        ...
        public CoroutineHandle RunCoroutineSingletonOnInstance(IEnumerator<float> coroutine, int layer, string tag, SingletonBehavior behaviorOnCollision)
        {
            ...
            if (behaviorOnCollision == SingletonBehavior.Wait)
            {
                ...
                if (matches.Count > 0)
                {
                    var newCoroutineHandle = RunCoroutineInternal(coroutine, Segment.Update, layer, true, tag, new CoroutineHandle(_instanceID), false);
                    WaitForOtherHandles(newCoroutineHandle, matches, false);
                    return newCoroutineHandle;
                }
            }
            ...
            else if (_layeredProcesses.ContainsKey(layer))
            {
                ...
                switch (behaviorOnCollision)
                {
                    ...
                    case SingletonBehavior.AbortAndUnpause:
                        ...
                        break;
                    case SingletonBehavior.Wait:
                        var newCoroutineHandle = RunCoroutineInternal(coroutine, segment, layer, true, null,
                            new CoroutineHandle(_instanceID), false);
                        WaitForOtherHandles(newCoroutineHandle, _layeredProcesses[layer], false);
                        return newCoroutineHandle;
                }
            ...
            else if (_taggedProcesses.ContainsKey(tag))
            {
                ...
                switch (behaviorOnCollision)
                {
                    ...
                    case SingletonBehavior.AbortAndUnpause:
                        ...
                        break;
                    case SingletonBehavior.Wait:
                        var newCoroutineHandle = RunCoroutineInternal(coroutine, segment, 0, false, tag,
                            new CoroutineHandle(_instanceID), false);
                        WaitForOtherHandles(newCoroutineHandle, _taggedProcesses[tag], false);
                        return newCoroutineHandle;
                }
            ...
            SingletonBehavior behaviorOnCollision)
        {
            ...
            else if (behaviorOnCollision == SingletonBehavior.Wait)
            {
                ...
                if (matches.Count > 0)
                {
                    var newCoroutineHandle = RunCoroutineInternal(coroutine, segment, layer, true, tag, new CoroutineHandle(_instanceID), false);
                    WaitForOtherHandles(newCoroutineHandle, matches, false);
                    return newCoroutineHandle;
                }
            }
        ...
        private CoroutineHandle RunCoroutineInternal(IEnumerator<float> coroutine, Segment segment, int layer, bool layerHasValue, string tag, CoroutineHandle handle, bool prewarm)
        {
            ...
            var cashedHandle = currentCoroutine;
            ...
            try
            {
                switch (segment)
                {
                    case Segment.Update:
                        ...
                        {
                            var oldProcArray = UpdateProcesses;
                            var oldPausedArray = UpdatePaused;
                            var oldHeldArray = UpdateHeld;
                    ...
                    case Segment.FixedUpdate:
                        ...
                        {
                            var oldProcArray = FixedUpdateProcesses;
                            var oldPausedArray = FixedUpdatePaused;
                            var oldHeldArray = FixedUpdateHeld;
                    ...
                    case Segment.LateUpdate:
                        ...
                        {
                            var oldProcArray = LateUpdateProcesses;
                            var oldPausedArray = LateUpdatePaused;
                            var oldHeldArray = LateUpdateHeld;
                    ...
                    case Segment.SlowUpdate:
                        ...
                        {
                            var oldProcArray = SlowUpdateProcesses;
                            var oldPausedArray = SlowUpdatePaused;
                            var oldHeldArray = SlowUpdateHeld;
                    ...
                    case Segment.RealtimeUpdate:
                        ...
                        {
                            var oldProcArray = RealtimeUpdateProcesses;
                            var oldPausedArray = RealtimeUpdatePaused;
                            var oldHeldArray = RealtimeUpdateHeld;
                    ...
                    case Segment.EditorUpdate:
                        ...
                        {
                            var oldProcArray = EditorUpdateProcesses;
                            var oldPausedArray = EditorUpdatePaused;
                            var oldHeldArray = EditorUpdateHeld;
                    ...
                    case Segment.EditorSlowUpdate:
                        ...
                        {
                            var oldProcArray = EditorSlowUpdateProcesses;
                            var oldPausedArray = EditorSlowUpdatePaused;
                            var oldHeldArray = EditorSlowUpdateHeld;
                    ...
                    case Segment.EndOfFrame:
                        ...
                        {
                            var oldProcArray = EndOfFrameProcesses;
                            var oldPausedArray = EndOfFramePaused;
                            var oldHeldArray = EndOfFrameHeld;
                    ...
                    case Segment.ManualTimeframe:
                        ...
                        {
                            var oldProcArray = ManualTimeframeProcesses;
                            var oldPausedArray = ManualTimeframePaused;
                            var oldHeldArray = ManualTimeframeHeld;
        ...
        {
            var retVal = _nextUpdateProcessSlot + _nextLateUpdateProcessSlot + _nextFixedUpdateProcessSlot + _nextSlowUpdateProcessSlot +
        ...
        public static TimeTunnel GetInstance(byte ID)
        {
            if (ID >= 0x10)
            {
                return null;
        ...
        private bool UpdateTimeValues(Segment segment)
        {
            switch (segment)
            {
                ...
                case Segment.ManualTimeframe:
                    var timeCalculated = SetManualTimeframeTime == null ? Time.time : SetManualTimeframeTime(_lastManualTimeframeTime);
        ...
        public int PauseCoroutinesOnInstance()
        {
            ...
            while (indexesEnum.MoveNext())
            {
                if (!_handleToIndex.ContainsKey(indexesEnum.Current.Key))
                {
                    continue;
                }
        ...
        {
            var inst = GetInstance(handle.Key);
        ...
        public static bool SetSegment(CoroutineHandle handle, Segment newSegment)
        {
            ...
            var coindex = inst._handleToIndex[handle];
            var procPtr = inst.CoindexExtract(coindex);
            var coroutineHeld = inst.CoindexIsHeld(coindex);
            var coroutinePaused = inst.CoindexIsPaused(coindex);
        ...
        public static float WaitUntilDone(AsyncOperation operation)
        {
            ...
            var handle = CurrentCoroutine;
            var inst = GetInstance(CurrentCoroutine.Key);
            if (inst == null)
            {
                return float.NaN;
            }
        ...
        {
            var reciever = _tmpRef as System.Action<CoroutineHandle>;
            if (reciever != null)
            {
                reciever(handle);
            }
        ...
        {
            var handle = action == null ? new CoroutineHandle() : RunCoroutine(Instance._CallContinuously(period, action, null));
        ...
        {
            var handle = action == null ? new CoroutineHandle() : RunCoroutineOnInstance(_CallContinuously(period, action, null));
            if (!float.IsPositiveInfinity(timeframe))
            {
        ...
        {
            var handle = action == null ? new CoroutineHandle()
        ...
        {
            var handle = action == null ? new CoroutineHandle() : RunCoroutine(Instance._CallContinuously(0f, action, null));
            if (!float.IsPositiveInfinity(timeframe))
            {
        ...
        {
            var handle = action == null ? new CoroutineHandle() : RunCoroutineOnInstance(_CallContinuously(0f, action, null));
            if (!float.IsPositiveInfinity(timeframe))
            {
        ...
        {
            var handle = action == null ? new CoroutineHandle() : RunCoroutine(Instance._CallContinuously(0f, action, gameObject), gameObject);
            if (!float.IsPositiveInfinity(timeframe))
            {
                LinkCoroutines(handle, RunCoroutine(Instance._WatchCall(timeframe, handle, gameObject, onDone), gameObject));
            }
        ...
        {
            var handle = action == null ? new CoroutineHandle() : RunCoroutineOnInstance(_CallContinuously(0f, action, gameObject), gameObject);
            if (!float.IsPositiveInfinity(timeframe))
            {
                LinkCoroutines(handle, RunCoroutineOnInstance(_WatchCall(timeframe, handle, gameObject, onDone), gameObject));
            }
        ...
        {
            var handle = action == null ? new CoroutineHandle() : RunCoroutine(Instance._CallContinuously(0f, action, null), timing);
            if (!float.IsPositiveInfinity(timeframe))
            {
        ...
        {
            var handle = action == null ? new CoroutineHandle() : RunCoroutineOnInstance(_CallContinuously(0f, action, null), timing);
            if (!float.IsPositiveInfinity(timeframe))
            {
                LinkCoroutines(handle, RunCoroutineOnInstance(_WatchCall(timeframe, handle, null, onDone), timing));
            }
        ...
        {
            var handle = action == null ? new CoroutineHandle() : RunCoroutine(Instance._CallContinuously(reference, period, action, null));
            if (!float.IsPositiveInfinity(timeframe))
            {
        ...
        public CoroutineHandle CallPeriodicallyOnInstance<T>(T reference, float timeframe, float period,
            System.Action<T> action, System.Action<T> onDone = null)
        {
            var handle = action == null ? new CoroutineHandle() : RunCoroutineOnInstance(_CallContinuously(reference, period, action, null));
            if (!float.IsPositiveInfinity(timeframe))
            {
        ...
        {
            var handle = action == null ? new CoroutineHandle() : RunCoroutine(Instance._CallContinuously(reference, 0f, action, null));
            if (!float.IsPositiveInfinity(timeframe))
            {
                LinkCoroutines(handle, RunCoroutine(Instance._WatchCall(reference, timeframe, handle, null, onDone)));
            }
        ...
        {
            var handle = action == null ? new CoroutineHandle() : RunCoroutineOnInstance(_CallContinuously(reference, 0f, action, null));
            if (!float.IsPositiveInfinity(timeframe))
            {
                LinkCoroutines(handle, RunCoroutineOnInstance(_WatchCall(reference, timeframe, handle, null, onDone)));
            }
    ...
    public struct CoroutineHandle : System.IEquatable<CoroutineHandle>
    {
        ...
        {
            if (TimeTunnel.GetTag(this) == null)
            {
                if (TimeTunnel.GetLayer(this) == null)
                {
                    return TimeTunnel.GetDebugName(this);
                }
                else
                {
                    return TimeTunnel.GetDebugName(this) + " Layer: " + TimeTunnel.GetLayer(this);
                }
            }
            else
            {
                if (TimeTunnel.GetLayer(this) == null)
                {
                    return TimeTunnel.GetDebugName(this) + " Tag: " + TimeTunnel.GetTag(this);
                }
                else
                {
                    return TimeTunnel.GetDebugName(this) + " Tag: " + TimeTunnel.GetTag(this) + " Layer: " + TimeTunnel.GetLayer(this);
                }
            }
        }
        ...
        public string Tag
        {
            get => TimeTunnel.GetTag(this);
            set => TimeTunnel.SetTag(this, value);
        }
        ...
        public int? Layer
        {
            get => TimeTunnel.GetLayer(this);
            set
            {
                if (value == null)
                {
                    TimeTunnel.RemoveLayer(this);
                }
                else
                {
                    TimeTunnel.SetLayer(this, (int)value);
                }
            }
        }
        ...
        {
            get => TimeTunnel.GetSegment(this);
            set => TimeTunnel.SetSegment(this, value);
        }
        ...
        {
            get => TimeTunnel.IsRunning(this);
            set
            {
                if (!value)
                {
                    TimeTunnel.KillCoroutines(this);
                }
            }
        }
        ...
        {
            get => TimeTunnel.IsAliveAndPaused(this);
            set
            {
                if (value)
                {
                    TimeTunnel.PauseCoroutines(this);
                }
                else
                {
                    TimeTunnel.ResumeCoroutines(this);
                }
            }
        }
        ...
        {
            var inst = TimeTunnel.GetInstance(Key);
    ...
    public static class MECExtensionMethods1
    {
        ...
        {
            return TimeTunnel.RunCoroutine(coroutine);
        }
        ...
        {
            return TimeTunnel.RunCoroutine(coroutine, gameObj);
        }
        ...
        {
            return TimeTunnel.RunCoroutine(coroutine, layer);
        }
        ...
        {
            return TimeTunnel.RunCoroutine(coroutine, tag);
        }
        ...
        {
            return TimeTunnel.RunCoroutine(coroutine, gameObj, tag);
        }
        ...
        {
            return TimeTunnel.RunCoroutine(coroutine, layer, tag);
        }
        ...
        {
            return TimeTunnel.RunCoroutine(coroutine, segment);
        }
        ...
        {
            return TimeTunnel.RunCoroutine(coroutine, segment, gameObj);
        }
        ...
        {
            return TimeTunnel.RunCoroutine(coroutine, segment, layer);
        }
        ...
        {
            return TimeTunnel.RunCoroutine(coroutine, segment, tag);
        }
        ...
        {
            return TimeTunnel.RunCoroutine(coroutine, segment, gameObj, tag);
        }
        ...
        {
            return TimeTunnel.RunCoroutine(coroutine, segment, layer, tag);
        }
        ...
        {
            return TimeTunnel.RunCoroutineSingleton(coroutine, handle, behaviorOnCollision);
        }
        ...
        {
            return gameObj == null ? TimeTunnel.RunCoroutine(coroutine) :
                TimeTunnel.RunCoroutineSingleton(coroutine, gameObj.GetInstanceID(), behaviorOnCollision);
        }
        ...
        {
            return TimeTunnel.RunCoroutineSingleton(coroutine, layer, behaviorOnCollision);
        }
        ...
        {
            return TimeTunnel.RunCoroutineSingleton(coroutine, tag, behaviorOnCollision);
        }
        ...
        {
            return gameObj == null ? TimeTunnel.RunCoroutineSingleton(coroutine, tag, behaviorOnCollision)
                : TimeTunnel.RunCoroutineSingleton(coroutine, gameObj.GetInstanceID(), tag, behaviorOnCollision);
        }
        ...
        {
            return TimeTunnel.RunCoroutineSingleton(coroutine, layer, tag, behaviorOnCollision);
        }
        ...
        public static CoroutineHandle RunCoroutineSingleton(this IEnumerator<float> coroutine, CoroutineHandle handle, Segment segment,
            SingletonBehavior behaviorOnCollision)
        {
            return TimeTunnel.RunCoroutineSingleton(coroutine, handle, segment, behaviorOnCollision);
        }
        ...
        public static CoroutineHandle RunCoroutineSingleton(this IEnumerator<float> coroutine, Segment segment, GameObject gameObj,
            SingletonBehavior behaviorOnCollision)
        {
            return gameObj == null ? TimeTunnel.RunCoroutine(coroutine, segment) :
                TimeTunnel.RunCoroutineSingleton(coroutine, segment, gameObj.GetInstanceID(), behaviorOnCollision);
        }
        ...
        public static CoroutineHandle RunCoroutineSingleton(this IEnumerator<float> coroutine, Segment segment, int layer,
            SingletonBehavior behaviorOnCollision)
        {
            return TimeTunnel.RunCoroutineSingleton(coroutine, segment, layer, behaviorOnCollision);
        }
        ...
        public static CoroutineHandle RunCoroutineSingleton(this IEnumerator<float> coroutine, Segment segment, string tag,
            SingletonBehavior behaviorOnCollision)
        {
            return TimeTunnel.RunCoroutineSingleton(coroutine, segment, tag, behaviorOnCollision);
        }
        ...
        public static CoroutineHandle RunCoroutineSingleton(this IEnumerator<float> coroutine, Segment segment, GameObject gameObj, string tag,
            SingletonBehavior behaviorOnCollision)
        {
            return gameObj == null ? TimeTunnel.RunCoroutineSingleton(coroutine, segment, tag, behaviorOnCollision)
                : TimeTunnel.RunCoroutineSingleton(coroutine, segment, gameObj.GetInstanceID(), tag, behaviorOnCollision);
        }
        ...
        public static CoroutineHandle RunCoroutineSingleton(this IEnumerator<float> coroutine, Segment segment, int layer, string tag,
            SingletonBehavior behaviorOnCollision)
        {
            return TimeTunnel.RunCoroutineSingleton(coroutine, segment, layer, tag, behaviorOnCollision);
        }
        ...
        {
            return TimeTunnel.WaitUntilDone(newCoroutine);
        }
        ...
        {
            return TimeTunnel.WaitUntilDone(newCoroutine, tag);
        }
        ...
        {
            return TimeTunnel.WaitUntilDone(newCoroutine, layer);
        }
        ...
        {
            return TimeTunnel.WaitUntilDone(newCoroutine, layer, tag);
        }
        ...
        {
            return TimeTunnel.WaitUntilDone(newCoroutine, segment);
        }
        ...
        {
            return TimeTunnel.WaitUntilDone(newCoroutine, segment, tag);
        }
        ...
        {
            return TimeTunnel.WaitUntilDone(newCoroutine, segment, layer);
        }
        ...
        {
            return TimeTunnel.WaitUntilDone(newCoroutine, segment, layer, tag);
        }
    }

    public static class SuperCoroutinesExtensionMethods
    {
        /// <summary>
        ...
        {
            yield return TimeTunnel.WaitForSeconds(timeToDelay);
        ...
        {
            while (TimeTunnel.MainThread != System.Threading.Thread.CurrentThread ||
        ...
        {
            while (TimeTunnel.MainThread != System.Threading.Thread.CurrentThread || (gameObject1 && gameObject1.activeInHierarchy &&
        ...
        public static IEnumerator<float> CancelWith(this IEnumerator<float> coroutine, System.Func<bool> condition)
        {
            ...
            while (TimeTunnel.MainThread != System.Threading.Thread.CurrentThread || (condition() && coroutine.MoveNext()))
            {
                yield return coroutine.Current;
            }
        }
        ...
        {
            while (TimeTunnel.MainThread == System.Threading.Thread.CurrentThread && gameObject)
            {
        ...
        {
            while (TimeTunnel.MainThread == System.Threading.Thread.CurrentThread && gameObject1 && gameObject2)
            {
        ...
        public static IEnumerator<float> PauseWith<T>(this IEnumerator<float> coroutine, T script) where T : MonoBehaviour
        {
            ...
            while (TimeTunnel.MainThread == System.Threading.Thread.CurrentThread && myGO && myGO.GetComponent<T>() != null)
            {
        ...
        {
            return Superimpose(coroutineA, coroutineB, TimeTunnel.Instance);
        }
        ...
        /// <returns>The modified coroutine handle.</returns>
        public static IEnumerator<float> Superimpose(this IEnumerator<float> coroutineA, IEnumerator<float> coroutineB, TimeTunnel instance)
        {
